...
const baseFetchPath: string = 'https://www.unpkg.com/ts-auto-mock@3.2.2/';

export async function getContentAndDependenciesContent(file: string = './index.d.ts', alreadyFetched: {} = {}): Promise<{ content: string, path: string }[]> {
  if (alreadyFetched[file]) {
    return [];
  }

  console.info('Fetch dependency', file);
